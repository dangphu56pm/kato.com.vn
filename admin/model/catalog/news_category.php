	...
	public function addNewsCategory($data) {
		...
		if (isset($data['cat_layout'])) {
			foreach ($data['cat_layout'] as $store_id => $layout) {
				if ($layout['layout_id']) {
					$this->db->query("INSERT INTO " . DB_PREFIX . "news_category_to_layout SET news_category_id = '" . (int)$news_category_id . "', layout_id = '" . (int)$layout['layout_id'] . "'");
				}
			}
		}
