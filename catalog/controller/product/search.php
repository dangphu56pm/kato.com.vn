...
class ControllerProductSearch extends Controller { 	
	...
	// var_dump($this->session->data['sql']);
		...
		$this->load->model('catalog/news');
			...
			$news_total = $this->model_catalog_news->getTotalNews($data_news); 
			
			/* $results = $this->model_catalog_news->getNews($data_news); */
			
			foreach ($this->model_catalog_news->getNews($data_news) as $result) {
				...
				$short_description = strlen($result['description']) > 200 ? substr($description, 0, strrpos($description, ' '))."..." : substr($short_description, 0, strrpos($short_description, ' '));
		...
		if (isset($this->request->get['search']) || isset($this->request->get['tag']) || isset($this->request->get['category_id']) ) {
			...
			foreach ($results as $result) {
				...
				// if ((float)$result['special']) {
					...
					// $text_discount = ((((int)$result['special'] - (int)$result['price'])/(int)$result['price'])*100).'%';		
                 // }
			...
			$pagination_news->total = $news_total;
			$pagination_news->page = $page;
			$pagination_news->limit = $limit;
			$pagination_news->text = $this->language->get('text_pagination');
			$pagination_news->url = $this->url->link('product/search', $url . '&page={page}');

			$this->data['pagination_news'] = $pagination_news->render();
		}	
