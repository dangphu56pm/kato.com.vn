...
class ControllerNewsNewsCategory extends Controller {  
	public function index() {
		...
		if ($news_category_info) {
			if (strlen($news_category_info['meta_title']) > 1 ) {
				$title = $news_category_info['meta_title'];
			}else{
				$title = $news_category_info['name'];
			}
			$this->document->setTitle($title);
			$this->document->setMetanames('description',$news_category_info['meta_description']);
			if (strlen($news_category_info['meta_description']) > 1) {
                $meta_description = $news_category_info['meta_description'];
            } else {
                $meta_description = strip_tags(html_entity_decode($news_category_info['description'], ENT_QUOTES, 'UTF-8'));
            }

			
			$sitename= $this->config->get('config_name');
			
	  		$current_url = $this->url->link('news/news_category', 'cat_id=' . $news_category_id);
	  		
			//open graph
            $this->document->setMetapros('og:site_name',$sitename);
            $this->document->setMetapros('og:locate',$this->language->get('locate'));
            $this->document->setMetapros('og:type','object');
            ...
            $this->document->setMetapros('og:image',HTTP_SERVER.'image/' . $news_category_info['image']);
			...
			foreach ($results as $result) {
				...
				$short_description = strlen($result['description']) > 500 ? substr($description, 0, strrpos($description, ' '))."..." : substr($description, 0, strrpos($description, ' '));
				...
				$this->data['news'][] = array(
					...
					'date_day'       => date('d', strtotime($result['date_added'])),					
					'date_year'       => date('Y', strtotime($result['date_added'])),					
					'date_month'       => date('m', strtotime($result['date_added'])),					
					'width'      => $width, 
					'height'      => $height,
