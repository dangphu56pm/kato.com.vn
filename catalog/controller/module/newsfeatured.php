		...
		foreach ($newss as $news_id) {
			...
			if ($news_info) {			
				...
				if ($news_info['image'] && $setting['imagestatus'] == 1) {
					...
					$imagesmall = $this->model_tool_image->resize($news_info['image'], $setting['image_width']/2, $setting['image_height']/2);
					...
					if($firstImgNews == 'no_image.jpg'){
						...
						$imagesmall = $this->model_tool_image->resize('no_image.jpg', $setting['image_width']/2, $setting['image_height']/2);
					} else {
				...
				$this->data['newss'][] = array(
					...
					'sthumb'   	 => $imagesmall,
					'name'    	 => $news_info['name'],
					'date_added'    	 =>  date($this->language->get('date_format_short'), strtotime($news_info['date_added'])),
