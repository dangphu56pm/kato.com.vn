...
class ModelCatalogCategory extends Model {
    ...
    public function getCategoryNews($category_id) {
        ...
        $query = $this->db->query("SELECT * FROM " . DB_PREFIX . "category_related pn LEFT JOIN " . DB_PREFIX . "news n ON (n.news_id = pn.news_id) WHERE pn.category_id = '" . (int) $category_id . "' AND n.status = '1' AND n.date_available <= NOW()");
    ...
    public function getAllCategoriesByTop() {
        $query = $this->db->query("SELECT * FROM " . DB_PREFIX . "category c LEFT JOIN " . DB_PREFIX . "category_description cd ON (c.category_id = cd.category_id)  WHERE c.top='1' AND cd.language_id = '" . (int) $this->config->get('config_language_id') . "'  AND c.status = '1' ORDER BY c.sort_order, LCASE(cd.name)");
