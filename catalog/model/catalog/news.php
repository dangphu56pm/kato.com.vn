...
class ModelCatalogNews extends Model {
    ...
    public function getNews($data = array()) {
        if (!empty($data['filter_name_news'])) {
            $search = ", MATCH(nd.name) AGAINST ('" . $this->db->escape(utf8_strtolower($data['filter_name_news'])) . "' IN NATURAL LANGUAGE MODE) AS relevance ";
            $sort_search = " relevance DESC, ";
        } else {
            $search = '';
            $sort_search = '';
        }
        $sql = "SELECT n.news_id" . $search . " FROM " . DB_PREFIX . "news n LEFT JOIN " . DB_PREFIX . "news_description nd ON (n.news_id = nd.news_id) WHERE nd.language_id = '" . (int) $this->config->get('config_language_id') . "' AND n.status = '1' AND n.date_available <= NOW() ";

        if (isset($data['filter_name_news']) && $data['filter_name_news']) {
            // if (isset($data['filter_description']) && $data['filter_description']) {
            // $sql .= " AND (LOWER(nd.name) LIKE LOWER('%" . $this->db->escape($data['filter_name_news']) . "%') OR n.news_id IN (SELECT nt.news_id FROM " . DB_PREFIX . "news_tag nt WHERE nt.language_id = '" . (int)$this->config->get('config_language_id') . "' AND LOWER(nt.tag) LIKE LOWER('%" . $this->db->escape($data['filter_name_news']) . "%')) OR LOWER(nd.short_description) LIKE LOWER('%" . $this->db->escape($data['filter_name_news']) . "%') OR LOWER(nd.description) LIKE LOWER('%" . $this->db->escape($data['filter_name_news']) . "%'))";
            // } else {
            // $sql .= " AND (LOWER(nd.name) LIKE LOWER('%" . $this->db->escape($data['filter_name_news']) . "%') OR n.news_id IN (SELECT nt.news_id FROM " . DB_PREFIX . "news_tag nt WHERE nt.language_id = '" . (int)$this->config->get('config_language_id') . "' AND LOWER(nt.tag) LIKE LOWER('%" . $this->db->escape($data['filter_name_news']) . "%')))";
            // }
            $sql .= " AND MATCH(nd.name) AGAINST ('" . $this->db->escape(utf8_strtolower($data['filter_name_news'])) . "' IN NATURAL LANGUAGE MODE) <> 0 ";
        }


        // if (isset($data['search']) && $data['search']) {
        // $sql .= " AND (LOWER(nd.name) LIKE LOWER( '%" . $this->db->escape($data['search']) . "%') OR n.news_id IN (SELECT nt.news_id FROM " . DB_PREFIX . "news_tag nt WHERE nt.language_id = '" . (int)$this->config->get('config_language_id') . "' AND nt.tag LIKE '%" . $this->db->escape($data['search']) . "%') OR LOWER(nd.short_description) LIKE LOWER( '%" . $this->db->escape($data['search']) . "%') OR LOWER(nd.description) LIKE LOWER ('%" . $this->db->escape($data['search']) . "%'))";
        // } else {
        // $sql .= " AND (LOWER(nd.name) LIKE LOWER( '%" . $this->db->escape($data['search']) . "%') OR n.news_id IN (SELECT nt.news_id FROM " . DB_PREFIX . "news_tag nt WHERE nt.language_id = '" . (int)$this->config->get('config_language_id') . "' AND LOWER(nt.tag) LIKE LOWER ('%" . $this->db->escape($data['search']) . "%')))";
        // }
        ...
        if (isset($data['no_filter']) && $data['no_filter']) {
            $sql .= " AND n.news_id NOT IN (SELECT news_id FROM " . DB_PREFIX . "news_to_category WHERE news_category_id IN (".$data['no_filter'].")) ";
        }
        ...
        if (isset($data['sort']) && in_array($data['sort'], $sort_data)) {
            if ($data['sort'] == 'nd.name') {
                $sql .= " ORDER BY " . $sort_search . "n.sort_order, LCASE(" . $data['sort'] . ")";
            } else {
                $sql .= " ORDER BY " . $sort_search . "n.sort_order, " . $data['sort'];
            }
        } else {
...
//         print_r($sql);
    ...
    public function getNewsRelated($news_id) {
        ...
        $query = $this->db->query("SELECT * FROM " . DB_PREFIX . "news_related nr LEFT JOIN " . DB_PREFIX . "news n ON (nr.related_id = n.news_id)  WHERE nr.news_id = '" . (int) $news_id . "' AND n.status = '1' AND n.date_available <= NOW() ORDER BY n.sort_order ASC, n.date_added DESC");
    ...
    public function getOtherNews($news_category_id, $news_id, $limit) {
        ...
        if (empty($news_category_id)) {
            $sql = "SELECT n.news_id FROM " . DB_PREFIX . "news n  WHERE n.status = '1' AND n.date_available <= NOW() AND n.news_id != " . $news_id . " AND n.news_id < " . $news_id . " ORDER BY n.sort_order ASC, n.date_added DESC LIMIT " . (int) $limit;
        } else {
            $sql = "SELECT n.news_id FROM " . DB_PREFIX . "news n  LEFT JOIN " . DB_PREFIX . "news_to_category n2c ON (n.news_id = n2c.news_id) WHERE n.status = '1' AND n.date_available <= NOW() AND n2c.news_category_id = '" . (int) $news_category_id . "' AND n.news_id != " . $news_id . " AND n.news_id < " . $news_id . " ORDER BY n.sort_order ASC, n.date_added DESC  LIMIT " . (int) $limit;
        }
        // print_r($sql);
        $query = $this->db->query($sql);
    ...
    public function getTotalNews($data = array()) {
        ...
        $sql = "SELECT COUNT(*) AS total FROM " . DB_PREFIX . "news n LEFT JOIN " . DB_PREFIX . "news_description nd ON (n.news_id = nd.news_id)  WHERE nd.language_id = '" . (int) $this->config->get('config_language_id') . "' AND n.status = '1' AND n.date_available <= NOW()";

        if (isset($data['filter_name_news'])) {
            // if (isset($data['filter_description']) && $data['filter_description']) {
            // $sql .= " AND (nd.name LIKE LOWER('%" . $this->db->escape($data['filter_name_news']) . "%') OR n.news_id IN (SELECT nt.news_id FROM " . DB_PREFIX . "news_tag nt WHERE nt.language_id = '" . (int)$this->config->get('config_language_id') . "' AND nt.tag LIKE LOWER('%" . $this->db->escape($data['filter_name_news']) . "%')) OR nd.short_description LIKE '%" . $this->db->escape(strtolower($data['filter_name_news'])) . "%' OR nd.description LIKE '%" . $this->db->escape(strtolower($data['filter_name_news'])) . "%')";
            // } else {
            // $sql .= " AND (nd.name LIKE LOWER('%" . $this->db->escape($data['filter_name_news']) . "%') OR n.news_id IN (SELECT nt.news_id FROM " . DB_PREFIX . "news_tag nt WHERE nt.language_id = '" . (int)$this->config->get('config_language_id') . "' AND nt.tag LIKE LOWER('%" . $this->db->escape($data['filter_name_news']) . "%')))";
            // }
            $sql .= " AND MATCH(nd.name) AGAINST ('" . $this->db->escape($data['filter_name_news']) . "' IN NATURAL LANGUAGE MODE) <> 0 ";
        }
        // if (isset($data['search']) && $data['search']) {
        // $sql .= " AND (LOWER(nd.name) LIKE LOWER( '%" . $this->db->escape($data['search']) . "%') OR n.news_id IN (SELECT nt.news_id FROM " . DB_PREFIX . "news_tag nt WHERE nt.language_id = '" . (int)$this->config->get('config_language_id') . "' AND nt.tag LIKE '%" . $this->db->escape($data['search']) . "%') OR LOWER(nd.short_description) LIKE LOWER( '%" . $this->db->escape($data['search']) . "%') OR LOWER(nd.description) LIKE LOWER ('%" . $this->db->escape($data['search']) . "%'))";
        // } else {
